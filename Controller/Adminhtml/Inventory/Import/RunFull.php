 ...
 * RunFull Controller
 */
class RunFull extends Action implements HttpGetActionInterface
{
    ...
    /**
     * Execute Run Full inventory action
     */
        ...
        if ($this->helper->isInventoryImportEnabled($this->helper::INVENTORY_TYPE_FULL)) {
            ...
            if ($result) {
                $this->messageManager->addSuccessMessage(
                    __('Full inventory import has been finished.')
                );
            } else {
                $this->messageManager->addErrorMessage(
                    __('Unable to import full inventory.')
                );
            }
        }
